...
contract CollectionErc721 is ERC721, AccessControl {
    ...
    function addMintAdmin(address _newMintAdminAddress) external MustBeAdmin {
        _grantRole(MINT_ADMIN_ROLE, _newMintAdminAddress);
    }
    function removeMintAdmin(address _mintAdminAddress) external MustBeAdmin {
        _revokeRole(MINT_ADMIN_ROLE, _mintAdminAddress);
    }

    function activateCollection(string[] memory _uris, address _saleManager) external MustBeAdmin {
    ...
    // ******************************************* Mint Function **************************************************************************
    function mint(address _receiver) external MustBeMintAdmin returns (uint256) {
